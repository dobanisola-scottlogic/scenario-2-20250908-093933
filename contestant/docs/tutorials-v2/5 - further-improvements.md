...
if (direction.isPresent()){
    ...
    Direction opposite = d.getOpposite();   // Gets the opposite direction
}
```
...
uses A\* to calculate the route, but it is advised to only use calculate routes with this method for short distances, else your bot may take longer than 0.5s.
