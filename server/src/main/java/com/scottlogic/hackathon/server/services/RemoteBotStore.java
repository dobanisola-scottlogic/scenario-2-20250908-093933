...
/**
 *  The RemoteBotStore maintains a map that for each Team that have a Bot connected remotely contains a
 *  {@linkplain RemoteBotConnector} object.   The {@linkplain RemoteBotConnector} holds a reference to the
 *  {@linkplain RemoteBot} and the current State of the connection.
 *  If a Team connects or reconnects after a dropped connection then a new UUID is assigned to a new RemoteBot which is
 *  then updated in the RemoteBotStore and the UUID for the current Team bot updated in the {@linkplain BotStore}.
 *
 */
@Singleton
public class RemoteBotStore implements ChangeEventListener<RemoteBotChangeEvent> {
    ...
    public Optional<? extends Bot> get(Id id) {
        return teamBotMap.values().stream().map(RemoteBotConnector::getRemoteBot)
    ...
    RemoteBotConnector.State getConnectionState(String teamName) {
        RemoteBotConnector.State state = ofNullable(teamBotMap.get(teamName))
                ...
                .orElseThrow(() -> new RuntimeException("Attempt to get RemoteBotConnector.State for unknown remote team " + teamName));
