...
public class FastExpansionBot extends Bot {
        ...
        gameState.getPlayers().forEach(player -> {
            ...
            if (player.getOwner().equals(getId())) {
                if (!previousPlayers.contains(player.getId())) {
                    //create an ai for it.
                    agents.add(new BasicAgent(player.getId(), getNode(player.getPosition().getX(), player.getPosition().getY())));
                } else {
                    agents.forEach(agent -> {
    ...
    void updateMap(final GameState gameState) {
        ...
        //update collectables
        gameState.getCollectables().forEach(collectable -> {
            Node node = getNode(collectable.getPosition().getX(), collectable.getPosition().getY());
        ...
        //update water Nodes
        gameState.getOutOfBoundsPositions().forEach(position -> {
        ...
        //update hiveNodes
        gameState.getSpawnPoints().forEach(spawnPoint -> {
            if (!spawnPoint.getOwner().equals(getId())) {
                Node hive = getNode(spawnPoint.getPosition().getX(), spawnPoint.getPosition().getY());
    ...
    private boolean makeAgentExplore(BasicAgent agent, int range, boolean firstPass) {
        ...
        for (Entry<Node, Integer> entry : values.entrySet()) {
            if (!entry.getKey().isWater() && !entry.getKey().isHive() && entry.getValue() > bestValue && !entry.getKey().isOccupied()) {
    ...
    private void collectFood() {
        ...
        for (Node food : foods) {
            ...
            while (!openList.isEmpty()) {
                ...
                if (node.getDistance() > 2 && isEnemyNearFood.get(node.getSource())) {
                ...
                } else if (node.getPrevious() != null && node.isOccupied() && !node.getAgent().isFinished() && !node.getPrevious().isWater() && !node.getPrevious().isOccupied()) {
                    if (node.getPrevious() == node.getSource()) {
                        ...
                        if (closeEnemy != null) {
                            Collection<BasicAgent> closeAgents = getCloseAgentDistances(node.getSource()).values();
