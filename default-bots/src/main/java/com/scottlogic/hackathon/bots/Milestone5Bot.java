...
public class Milestone5Bot extends Bot {
    ...
    public List<Move> makeMoves(final GameState gameState) {
        ...
        collectablePositions = gameState.getCollectables().stream()
    ...
    private Move doMove(final GameState gameState, final List<Position> nextPositions, final Player player) {
        ...
        Move lastMove = lastMoves == null ? null : lastMoves.stream().filter(move -> player.getId().equals(move.getPlayer())).findAny().orElse(null);
        Direction direction = lastMove == null ? null : lastMove.getDirection();

        while (direction == null || (!directions.isEmpty() && !canMove(gameState, nextPositions, player, direction))) {
            direction = directions.remove(ThreadLocalRandom.current().nextInt(directions.size()));
        }
        return new MoveImpl(player.getId(), direction);
    }

    private Move doCollect(final GameState gameState, final Player player, final List<Position> nextPositions) {
        int distance;
        ...
        for (Position collectablePosition : collectablePositions) {
            int collectableDistance = gameState.getMap().distance(player.getPosition(), collectablePosition);
        ...
        if (closestCollectable != null) {
            Optional<Direction> direction = gameState.getMap().directionsTowards(player.getPosition(), closestCollectable).findFirst();
