...
public class RemoteBotSocketProtocol {
    ...
    void receive(String message) {
        try {
            logger.debug("{} onMessage Server Bot:" + (bot == null ? "#" : bot.getId()) + " STATE IS: {} RECEIVED: {}", getThreadName(), state, message);
            switch (state) {
                case WAITING: {
                    createBot(message);
    ...
    void createBot(String input) throws IOException {
        ...
        logger.debug("{} CREATE BOT TEAM NAME: {} ID: {} STATE IS: {}", getThreadName(), teamId.getName(), teamId.getId(), state);
    }

    void notifyBotDisconnected() {
        logger.trace("{} firing propertyChange for propertyName: {} to {} listeners",
                getThreadName(), bot.getDisplayName(), changeSupport.count());
        changeSupport.fireChangeEvent(new ConnectionChangeEvent(bot.getDisplayName(), new RemoteBotCallback(bot, sender), null));
    ...
    private void notifyBotCreated(String team, RemoteBot bot) throws IOException {
        ...
        if (hasListeners(team)) {
            logger.debug("{} firing propertyChange for propertyName: {} to {} listeners",
                    getThreadName(), team, changeSupport.count());
            changeSupport.fireChangeEvent(new ConnectionChangeEvent(bot.getDisplayName(), null, new RemoteBotCallback(bot, sender)));
        } else {
            logger.warn("Not expecting connection for team {} - Has team been defined by Admin? Or has the contestant selected connect?", team);
