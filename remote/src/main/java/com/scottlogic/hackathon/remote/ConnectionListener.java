 ...
 * <p>
 * When there is a new connection for the Team associated with the connector calls {@link RemoteBotConnector#connected(RemoteBotCallback)}
 * When there is a disconnect for the Team associated with the connector calls {@link RemoteBotConnector#waitForConnect(String)}
 * <p>
 * When the same team connects multiple times then the second connection causes the first to  be disconnected
 */
public class ConnectionListener implements ChangeEventListener<ConnectionChangeEvent> {
    ...
    private void sendClientDisconnect(String team, Object newValue, Object oldValue) {
        try {
            ...
            logger.warn("unable to send client disconnect message to {} : {}", team, ioe.getLocalizedMessage());
        }
    }
