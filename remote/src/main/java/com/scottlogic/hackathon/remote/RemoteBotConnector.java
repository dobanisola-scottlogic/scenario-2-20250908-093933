...
/**
 * The RemoteBotConnector provides ability to block until a connection is made by a remote team {@link RemoteBotConnector#getTeam()}
 * Interested parties such as the {@linkplain com.scottlogic.hackathon.server.services.RemoteBotStore} can register a listener
 * to be notified when a connection in made.
 * The {@link RemoteBotConnector#getState()} returns the current state of the connection to the remote contestants bot.
 * The {@linkplain State#WAITING} indicates the {@link RemoteBotConnector} is awaiting a connection from the contestant bot
 * The {@linkplain State#CONNECTED} indicates the {@link RemoteBotConnector} is has a connection with the contestant bot
 */
public class RemoteBotConnector {

    public enum State {WAITING, CONNECTED}
    ...
    public Id getId() { return botCallback.getBot().getId(); }
